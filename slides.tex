...
% Frame title with a thin gray rule underneath
...
\begin{frame}{Data and setting}
  ...
  \begin{itemize}
    \item \textbf{Cash leg}\quad \todohl{source to be decided}
  ...
  \textbf{Coverage}
  \begin{itemize}
    \item SFTDS and EMIR report every transaction that involves at least one euro area entity.
    \item Euro area dealers are global banks, so the data cover both EUR and USD markets.
...
\begin{frame}{Measuring basis positions}
  \begin{itemize}
    \item Match the three legs at fund level and classify basis versus directional books.
    \item Validate the aggregates against OFR positions and published ECB and Fed estimates.
  \end{itemize}
  % TODO: validation figure (e.g. \includegraphics[width=.7\textwidth]{ofrcomp.pdf})
\end{frame}
