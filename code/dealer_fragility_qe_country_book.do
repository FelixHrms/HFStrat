...
cap log close
log using "$key\\dealer_fragility_qe_country_book.log", replace text

**# Quarter-end window dressing by dealer and collateral country
* mirrors dealer_fragility_qe_country.do with the treatment built per collateral
* country, dress = the dealer's contraction of its non hedge fund repo book in
* that country's government bonds, specific repo only since GC has no country
...
**# Step 2: dealer treatment by collateral country
* dress = log of the daily book in the reference window minus log of the daily
* book in the event window, per dealer and collateral country, positive means
* the dealer shrinks its book in that country's bonds at quarter-end

import delimited "$key\\dealer_book_country_day.csv", varnames(1) clear
capture drop v1
gen date = date(business_date, "YMD")
...
collapse (sum) book, by(dealer_id country quarter window)
reshape wide book, i(dealer_id country quarter) j(window)
replace book0 = book0/(`R1' - `R0' + 1) /*daily averages*/
replace book1 = book1/`K'
gen dress = log(book0) - log(book1)
label var dress "Quarter-end contraction of the dealer's non HF repo book in the country's bonds"
drop if missing(dress) /*no book in one of the two windows, the dealer is not active in that country that quarter*/
tabstat dress, by(country) stat(mean sd n)
keep dealer_id country quarter dress
tempfile dealers
save `dealers'
...
merge m:1 dealer_id country quarter using `dealers', keep(match) nogen
...
preserve
	keep if e(sample) /*the dealer country quarters that identify the net regression*/
	collapse (first) dress, by(dealer_id country quarter)
	tabstat dress, by(country) stat(mean sd n)
restore
